import torch
from src.ops.sort_vertices.sort_vert import sort_v
EPSILON = 1e-8
...
def cal_iou(box1:torch.Tensor, box2:torch.Tensor):
    ...
    # TODO When using fp16, calculate iou with fp32.!!! otherwise will return nan
    inter_area, _ = oriented_box_intersection_2d(corners1.to(torch.float32), corners2.to(torch.float32))        #(B, N)
    area1 = box1[:, :, 2] * box1[:, :, 3]
    area2 = box2[:, :, 2] * box2[:, :, 3]
...
def box2corners(boxes):
    """
    Convert box parameters to four box corners.

    Args:
        boxes (torch.Tensor): Tensor of shape (B, N, 5) containing x, y, w, h, alpha.

    Returns:
        torch.Tensor: Tensor of shape (B, N, 4, 2) containing the corners of the boxes.
    """
    B, N, _ = boxes.shape
    x, y, w, h, alpha = boxes[..., 0], boxes[..., 1], boxes[..., 2], boxes[..., 3], boxes[..., 4]
    
    x4 = torch.tensor([0.5, -0.5, -0.5, 0.5], device=boxes.device) * w.unsqueeze(-1)
    y4 = torch.tensor([0.5, 0.5, -0.5, -0.5], device=boxes.device) * h.unsqueeze(-1)
    corners = torch.stack([x4, y4], dim=-1)  # Shape: (B, N, 4, 2)
    
    sin = torch.sin(alpha)
    cos = torch.cos(alpha)
    R = torch.stack([cos, -sin, sin, cos], dim=-1).reshape(B, N, 2, 2)  # Shape: (B, N, 2, 2)
    
    rotated = torch.einsum('bnij,bnjk->bnik', corners, R)  # Batch matrix multiplication, Shape: (B, N, 4, 2)
