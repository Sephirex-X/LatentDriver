...
class SpeedDivergenceMetric(abstract_metric.AbstractMetric):
  """Log divergence metric.
  ...
  @jax.named_scope('SpeedDivergenceMetric.compute')
  def compute(
      self, simulator_state: datatypes.SimulatorState
  ) -> abstract_metric.MetricResult:
    """Computes log divergence by fetching correct arguments.
    ...
    # jax.debug.print("{x},{y}", x=current_object_state.xy.shape, y=current_log_state.speed.shape)
    # print(current_object_state.xy.shape,current_object_state.speed.shape)
    # current_object_state.xy (bs,objs,1,2); current_object_state.speed (bs,objs,1)
    result = self.compute_log_divergence(
        current_object_state.speed[...,jnp.newaxis], current_log_state.speed[...,jnp.newaxis]
    )
    valid = current_object_state.valid & current_log_state.valid
    return abstract_metric.MetricResult.create_and_validate(
        result[..., 0], valid[..., 0]
