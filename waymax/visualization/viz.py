...
def plot_observation(
  ...
  if batch_idx > -1:
    if len(obs.shape) == 2:
      ...
      device_id = batch_idx // obs.shape[1]
      batch_idx = batch_idx % obs.shape[1]
      obs = jax.tree_util.tree_map(lambda x: x[device_id,batch_idx,...], obs)
  ...
  id_list = []
  pos_x = origin_x - viz_config.back_x
  pos_y = origin_y + viz_config.front_y
  if route_segments is not None:
    route_segments = route_segments[device_id,batch_idx]
